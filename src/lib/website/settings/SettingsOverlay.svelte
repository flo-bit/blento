<script lang="ts" module>
	export const settingsOverlayState = $state({
		visible: false,
		activeSection: 'layout' as string,
...
{#if settingsOverlayState.visible}
	<div class="bg-base-50 dark:bg-base-950 fixed inset-0 z-[100] flex flex-col overflow-hidden">
		...
		<div
			class="border-base-200 dark:border-base-800 flex items-center justify-between border-b px-6 py-4"
		>
			<div class="flex items-center gap-6">
				...
				<nav class="flex gap-1 overflow-x-auto">
					{#each tabs as tab (tab.id)}
						<button
							type="button"
							class="cursor-pointer rounded-lg px-3 py-1.5 text-sm font-medium whitespace-nowrap {settingsOverlayState.activeSection ===
							tab.id
		...
		<div class="flex-1 overflow-y-auto px-6 py-8">
			<div class="mx-auto max-w-xl">
				{#if settingsOverlayState.activeSection === 'layout'}
