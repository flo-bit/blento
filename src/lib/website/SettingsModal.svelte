<script lang="ts" module>
	export const settingsModalState = $state({
		visible: false,
		show: () => (settingsModalState.visible = true),
		hide: () => (settingsModalState.visible = false)
	});
</script>
...
<Modal bind:open={settingsModalState.visible}>
	<h3 class="text-base-900 dark:text-base-100 text-lg font-semibold">Settings</h3>

	<div class="mt-4">
		<h4 class="text-base-800 dark:text-base-200 text-sm font-medium">Layout Sync</h4>
		...
		<div class="mt-3 flex flex-col gap-2">
			{#each options as option (option.key)}
				...
				<button
					type="button"
					class="border-base-200 dark:border-base-700 hover:border-base-300 dark:hover:border-base-600 rounded-xl border-2 px-4 py-3 text-left transition-colors {isSelected
