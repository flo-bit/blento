...
{#if user.isLoggedIn && user.profile}
	<div
		class="bg-base-100 dark:bg-base-950 border-base-200 dark:border-base-800 fixed top-3 right-3 z-20 flex flex-col items-center gap-1 rounded-full border p-1.5 shadow-md"
	>
		<!-- Avatar with popover -->
		<Popover
			side="left"
			...
			class="bg-base-100 dark:bg-base-900"
		>
			{#snippet child({ props })}
				<button {...props} class="cursor-pointer">
					<Avatar src={user.profile?.avatar} alt="" class="size-8 rounded-full" />
				</button>
			{/snippet}
		...
		<button
			type="button"
			class="text-base-500 hover:text-base-700 dark:text-base-400 dark:hover:text-base-200 flex size-8 cursor-pointer items-center justify-center rounded-full transition-colors"
