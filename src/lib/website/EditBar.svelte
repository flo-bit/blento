...
{#if dev || (user.isLoggedIn && user.profile?.did === data.did)}
	...
	>
			...
			>
				<div class="flex items-center gap-0.5">
							...
							{#snippet child({ props })}
								<button
									...
									class={[
										...
										selectedColor?.class ?? 'text-base-800 dark:text-base-200'
									]}
								>
									<svg
					...
					<Popover bind:open={sizePopoverOpen}>
						{#snippet child({ props })}
							<button {...props} class="hover:bg-accent-500/10 cursor-pointer rounded-lg p-1">
								<svg
									xmlns="http://www.w3.org/2000/svg"
									fill="none"
									viewBox="0 0 24 24"
						...
						<div class="flex items-center gap-1">
							...
							{#if canSetSize(4, 2)}
								...
								>
									<div
										class="border-base-900 dark:border-base-50 h-3 w-5 rounded-sm border-2"
									></div>
							...
							{#if canSetSize(2, 4)}
								...
								>
									<div
										class="border-base-900 dark:border-base-50 h-5 w-3 rounded-sm border-2"
									></div>
					...
					{#if allowRotate}
						<button
							onclick={() => onrotate?.(-2)}
							class="hover:bg-accent-500/10 cursor-pointer rounded-lg p-1"
							aria-label="Rotate -2°"
						>
							<svg
								xmlns="http://www.w3.org/2000/svg"
		...
		<Navbar
			class="dark:bg-base-950 bg-base-100 relative top-auto mt-0 h-13 w-full max-w-3xl rounded-full px-4"
		>
			<div class="flex items-center gap-2">
				<Button size="icon" variant="ghost" class="backdrop-blur-none" onclick={showCardCommand}>
					<svg
						xmlns="http://www.w3.org/2000/svg"
						fill="none"
						viewBox="0 0 24 24"
