	...
	{#snippet controls()}
		...
		{#if canEdit()}
			...
			{#if item.hideOnMobile && isMobile()}
				<div
					class="bg-base-900/80 dark:bg-base-100/80 text-base-50 dark:text-base-900 absolute -top-3 -right-3 rounded-full px-2 py-1 text-xs font-medium shadow-lg"
				>
					Hidden on mobile
				</div>
			{/if}
			...
			>
				...
				>
					...
					<button
						onclick={() => {
							item.hideOnMobile = item.hideOnMobile ? undefined : true;
						}}
						class={[
							'hover:bg-accent-500/10 cursor-pointer rounded-xl p-2',
							item.hideOnMobile ? 'bg-accent-500/20' : ''
						]}
						title={item.hideOnMobile ? 'Show on mobile' : 'Hide on mobile'}
					>
						...
						{/if}
						<span class="sr-only">{item.hideOnMobile ? 'Show on mobile' : 'Hide on mobile'}</span>
					</button>
