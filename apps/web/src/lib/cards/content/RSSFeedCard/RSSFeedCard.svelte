	...
	{:else}
		<div class={item.cardData?.label ? 'pt-4' : ''}>
			{#if item.cardData?.title}
				<div class="mb-4">
					<h3 class="text-base-900 dark:text-base-50 truncate text-lg font-semibold">
						{item.cardData.title}
					</h3>
				</div>
			{/if}
			...
			{#if feed && feed.length > 0}
				{#each feed.slice(0, item.cardData?.itemCount ?? 5) as entry (entry.href)}
					<article class="group/article relative isolate flex flex-col">
						<div
							class="text-base-500 accent:text-accent-950 flex shrink-0 items-center gap-x-4 text-xs"
						>
						...
						<div class="max-w-xl">
							...
							{#if item.cardData?.showDescription !== false && entry.description}
								<p
									class="text-base-600 dark:text-base-400 accent:text-base-800 mt-5 text-sm leading-6"
								>
									{entry.description}
								</p>
							{/if}
			...
			{:else}
				<div
					class="z-50 flex h-full flex-col items-center justify-center gap-4 text-center text-sm"
				>
